#!/usr/bin/env python3
"""Generate excited Final Round win amount voiceovers (ElevenLabs v3)."""
...
CONFIG_PATH = ROOT / "data" / "vo-final-win-amounts.json"
OUT_DIR = ROOT / "assets" / "audio" / "vo" / "final-win"
...
def main() -> None:
    ...
    intro = config.get("intro", "You've won")
    lines = config["amounts"]
    ...
    for i, line in enumerate(lines):
        spoken = f"{prefix} {intro}... {line['text']}"
        out_path = OUT_DIR / f"{line['id']}.mp3"
        print(f"[{i + 1}/{len(lines)}] {line['value']}: {spoken!r}")
        ...
        manifest["clips"].append(
            {
                ...
                "text": line["text"],
                "spoken": spoken,
                "file": f"assets/audio/vo/final-win/{line['id']}.mp3",
            }
        )
        time.sleep(0.35)
